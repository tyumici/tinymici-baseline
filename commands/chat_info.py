...
class ChatInfo:
    ...
    async def youtube(cmd: ChatCommand):
        await cmd.reply(
            f"[{models.globals._BOT_SIGIL}] {os.getenv("YOUTUBE")}"
        )

    async def twitter(cmd: ChatCommand):
        await cmd.reply(
            f"[{models.globals._BOT_SIGIL}] {os.getenv("TWITTER")}"
        )

    async def x(cmd: ChatCommand):
        await cmd.reply(
            f"[{models.globals._BOT_SIGIL}] {os.getenv("X")}"
        )
    ...
    async def github(cmd: ChatCommand):
        await cmd.reply(
            f"[{models.globals._BOT_SIGIL}]{os.getenv("GITHUB")}"
        )
