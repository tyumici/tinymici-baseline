...
def reconnect_db_job():
    ...
    models.globals._connectionPrimary = DataService.reconnect_primary(models.globals._connectionPrimary)
    ...
    def connect_primary():
        try:
            ...
            if models.globals._connectionPrimary.is_connected():
                print(colored(f"Connected to MariaDB on test DB", 'green'))
    ...
    def reconnect_primary(connection):
        while True:
            try:
                ...
                print(colored(f"MariaDB Connection lost. Attempting to reconnect: {e}", 'yellow'))
                time.sleep(10)
...
# --- --- --- --- --- SECRET HANDLERS --- --- --- --- --- #
    ...
    def update_bot_tokens(token: str, refresh_token: str):
        try:
            ...
            cursor.executemany("UPDATE SECRETS SET 'VALUE' = ? WHERE 'TYPE' = ?", new_tokens)
